...
class TestMessageValidation:
    ...
    def test_message_quality_warnings(self):
        ...
        test_cases = [
            ...
            ("Contact me URGENT!!! ASAP!!!", "professional"),
        ...
        for message, expected_warning in test_cases:
            ...
            # Should still be valid but have warnings (except for very short which is valid)
            if expected_warning == "very short":
                assert result["valid"] == True
            else:
                # Check that either it's invalid or has warnings
                if not result["valid"]:
                    continue  # Invalid is fine too
...
class TestPhase2Integration:
    ...
    def test_phase1_tools_still_available_with_phase2(self):
...
class TestErrorHandling:
    ...
    async def test_message_validation_edge_cases(self):
        ...
        edge_cases = [
            ...
            (None, "follow_up")    # None input (will be caught by pydantic)
        ]
        ...
        for message, msg_type in edge_cases:
            with UserContext(user_id="john.smith", conversation_id="conv-123", user_type="employee"):
                try:
                    ...
                    # Pydantic validation errors are expected for None
                    assert "validation error" in str(e).lower() or message is None
